...
使用蓝图将创建一个 tlken，其中所有 Handler 和状态都已经定义。这是创建 token 最简单的方法。加载蓝图后，您可以自定义这些 Handler 和状态。

您可以在此处了解更多有关蓝图的信息：[蓝图](../aos/blueprints/index.md)

::: 信息
虽然使用 token 蓝图虽然可以快速创建，但您仍然需要了解如何进行[加载和测试](token.html#loading-and-testing), 以便根据需要进行自定义。
:::

**2 - 从零开始构建:**

以下指南将指导您完成从零开始创建 token。这是创建 token 的一种更高级的方法，可以让您更好地理解 token 的工作原理。
## 准备工作
### **步骤 1：初始化 Token**
...
此时，您可能已经注意到，我们正在 `token.lua` 文件中构建所有 Handler，而不是使用 `.editor`。
...
然后您会看到铸造 token 的进程 ID 的余额增加了。
## 结论

“构建 token ”指南到此结束。学习构建自定义 token 可以为您的项目释放巨大的潜力；无论您是想创建新货币、游戏 token 、治理 token ，还是任何其他您能想到的东西。